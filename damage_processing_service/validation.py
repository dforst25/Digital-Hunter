...
from model import DamageMessage
...
class MessageValidator:
    ...
    def is_attacked(self, target_id):
        if self.target_exists(target_id):
            target = self.mongo_client.get_target(target_id)
            return target.get("is_attacked", False)
        else:
            return False

    def validate(self, damage_message: dict):
        self.logger.info("the validation started...")
        try:
            message = DamageMessage(**damage_message).dict()
        ...
        if not self.is_attacked(target_id):
            raise ValueError(f"the target with id: {target_id} is still not attacked")