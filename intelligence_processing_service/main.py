...
def main():
    ...
    coll_name = os.getenv("COLL_NAME", "intel")
    mongo_client = MongoConnector(mongo_uri, coll_name, logger)
    bootstrap_servers = os.getenv("BOOTSTRAP_SERVERS", "localhost:9092")
    topic_name = os.getenv("TOPIC_NAME", "intel")
    error_topic_name = os.getenv("ERROR_TOPIC_NAME", "intel_signals_dlq")
    group_id = os.getenv("GROUP_ID", "intel-service")
    ...
    processor = Processor(logger, validator, mongo_client)
    while True:
        ...
        try:
            ...
            continue
        processor.intersect_data(message)
        processor.insert_movement_distance(message)
